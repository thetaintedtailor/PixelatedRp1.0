...
function OpenBoatsMenu(x, y , z)
    ...
    if data.current.value == 'fish_license' then
        ESX.TriggerServerCallback('esx_license:checkLicense', function(hasLicense)
            if (not hasLicense) then
                ESX.TriggerServerCallback('fishing:buy', function(success)
                    if (success) then
                        ESX.UI.Menu.CloseAll()
                        ...
                        TriggerEvent("chatMessage", "You don't have enough money.")
                    end
                end, Config.FishLicenseCost)
            else
