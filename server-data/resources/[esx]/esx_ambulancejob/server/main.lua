...
ESX.RegisterServerCallback('esx_ambulancejob:storeNearbyVehicle', function(source, cb, nearbyVehicles)
	...
	local fuel = nearbyVehicles[1].fuel
...
AddEventHandler('esx_ambulancejob:giveItem', function(itemName)
	...
	if xPlayer.job.name ~= 'ambulance' then
		...
		return
	elseif (itemName ~= 'medikit' and itemName ~= 'bandage'and itemName ~= 'morphine'and itemName ~= 'gauze'and itemName ~= 'medicaltape' and itemName ~= 'k3v' and itemName ~= 'vicadin' and itemName ~= 'ibuprofen' and itemName ~= 'amoxicillin' and itemName ~= 'xanax') then
		print(('esx_ambulancejob: %s attempted to spawn in an item!'):format(xPlayer.identifier))
		return
	end
