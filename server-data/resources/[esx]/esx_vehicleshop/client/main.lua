...
function OpenShopMenu()
	...
	for i=1, #Categories, 1 do
		...
		for j=1, #categoryVehicles, 1 do
			...
			local payments = math.ceil(vehicle.price / exports.pixelated_vehicle_financing:GetPaymentDays())
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), '', _U('car_dealer'), tonumber(data2.value))
					end
				end
			end, function (data2, menu2)
				menu2.close()
...
--[[if Config.EnablePlayerManagement then
	...
	AddEventHandler('esx_phone:loaded', function (phoneNumber, contacts)
		local specialContact = {
			name       = _U('dealership'),
			number     = 'cardealer',
			base64Icon = 'data:image/x-icon;base64',
		}
