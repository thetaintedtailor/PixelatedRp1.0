...
AddEventHandler('esx:playerLoaded', function(source)
	...
	MySQL.Async.fetchAll('SELECT phone_number FROM users WHERE identifier = @identifier', {
		...
		MySQL.Async.fetchAll('SELECT * FROM phone_users_contacts WHERE identifier = @identifier ORDER BY display ASC', {
			['@identifier'] = xPlayer.identifier
		}, function(result2)

			for i=1, #result2, 1 do
				print(result2[i].display .. ' - ' .. result2[i].number)
				table.insert(contacts, {
					name   = result2[i].display,
	...
	MySQL.Async.fetchAll('SELECT phone_number FROM users WHERE phone_number = @number', {
		...
		if result[1] ~= nil then
			...
			if phoneNumber == xPlayer.get('phoneNumber') then
				TriggerClientEvent('esx:showNotification', _source, 'You cannot add yourself')
			else
				...
				if hasAlreadyAdded then
					...
					MySQL.Async.execute('INSERT INTO phone_user_contacts (identifier, display, number) VALUES (@identifier, @display, @number)',
					{
						['@identifier'] = xPlayer.identifier,
						['@display']       = contactName,
						['@number']     = phoneNumber
			...
			TriggerClientEvent('esx:showNotification', source, 'Non-existent number')
		end
...
AddEventHandler('esx_phone:billCall', function(duration)
	...
	TriggerClientEvent('esx:showNotification', _source, 'The call cost ~r~$' .. amount .. '~s~')
end)
...
AddEventHandler('esx_phone:bankTransfer', function(target, amount)
	...
	if amount > 0 and amount <= xPlayer.getAccount('bank').money then
		...
		TriggerClientEvent('esx:showNotification', _source, 'You have fired ~g~$' .. amount .. '~s~ a ' .. targetXPlayer.name)
	else
